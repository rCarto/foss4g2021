---
title: "mapsf, a new package for thematic mapping"
author: "Timothée Giraud </br></br> ![](img/logocnrs.svg)       ![](img/logo_RIATE.png)</br>"
date: "FOSS4G 2021 -  2021/09/30"
output:
  ioslides_presentation: 
    css: libs/style.css
    logo: img/logo.png
    widescreen: yes
---

```{r setup, include=FALSE, cache=FALSE}
knitr::opts_chunk$set(echo = TRUE, cache=TRUE)
```


## {data-background=img/map2.svg data-background-size=contain}

```{r, echo = FALSE, message = FALSE, results='hide'}
library(mapsf)
mtq <- mf_get_mtq()
th <- mf_theme("default", fg = "grey20", bg = "grey80", mar = c(0,0,2,0), 
               line = 2, tab = FALSE, cex = 1.5)
mf_export(mtq, export = "svg", height = 7, res = 80,
          filename = "img/map2.svg", theme = th, family = "B612")
mf_background("img/marjan-blan-marjanblan-_kUxT8WkoeY-unsplash.jpg")
mf_shadow(mtq, add = TRUE)
mf_map(mtq, "MED", "choro",
       leg_title = "Median Income\n(in €)", 
       breaks = "equal",
       pal = "Emrld",
       nbreaks = 4,
       leg_pos = "topright", leg_val_rnd = -2, add = TRUE)
mf_title("Wealth in Martinique, 2015")
mf_credits(
  txt = paste0("Sources: Insee and IGN, 2018\n",
               "mapsf ",
               packageVersion("mapsf"),"\n",
               'Background photo by Marjan Blan on Unsplash'))
mf_arrow()
mf_scale(5)
dev.off()
```


## Typical workflow {data-background=img/workflow.png data-background-size=contain}


## `mf_map()`

[`mf_map()`](https://riatelab.github.io/mapsf/reference/mf_map.html) is the main function of the package.

```r 
mf_map(x = sf_object, 
       var = "variable", 
       type = "map type", 
       ...)
```

##  {data-background=img/help.png data-background-size=contain}
### `?mf_map()`


## 9 map types - [Vignette](https://riatelab.github.io/mapsf/articles/mapsf.html)

<center>
<img src="img/visu1.png" width=579px />
</center>


## 9 map types - [Vignette](https://riatelab.github.io/mapsf/articles/mapsf.html)
<center>
<img src="img/visu2.png" width=511px />
</center>



## Map layout

<center>
<img src="img/deco2.svg" width=850px />  
</center>


## Example {.smaller}

<div class="columns-2">
```{r, results='hide', message = FALSE, echo = FALSE}
library(mapsf)
```

```{r a0, eval=FALSE}
mtq <- mf_get_mtq()
# 
# 
#        
# 
# 
# Plot the base map
mf_map(x = mtq)
#         
#       
#        
#        
#        
#        
# 
# 
# 
# 
# 
# 
# 
# 
# 
#
```
```{r a0, message = FALSE, results='hide', fig.width = 5, fig.height=5, eval=TRUE, echo=FALSE, cache = TRUE}
```
</div>


## Example {.smaller}

<div class="columns-2">
```{r a, eval=FALSE}
mtq <- mf_get_mtq()
# 
# 
#        
# 
# 
# Plot a choropleth map
mf_map(x = mtq, var = "MED", type = "choro")
#         
#       
#        
#        
#        
#        
# 
# 
# 
# 
# 
# 
# 
# 
# 
#
```
```{r a, message = FALSE, results='hide', fig.width = 5, fig.height=5, eval=TRUE, echo=FALSE}
```
</div>


## Example {.smaller}

<div class="columns-2">
```{r b, eval=FALSE}
mtq <- mf_get_mtq()
# 
# 
#        
# 
# 
# Plot a choropleth map
mf_map(x = mtq, var = "MED", type = "choro",
       pal = "Dark Mint", 
       breaks = "quantile", 
       nbreaks = 6, 
       leg_title = "Median Income\n(euros)", 
       leg_val_rnd = -2, 
       leg_pos = "topright")     
#        
# 
# 
# 
# 
# 
# 
#
# 
# 
```
```{r b, message = FALSE, results='hide', fig.width = 5, fig.height=5, eval=TRUE, echo=FALSE}
```
</div>



## Example {.smaller}

<div class="columns-2">
```{r c, eval=FALSE}
mtq <- mf_get_mtq()
# 
# 
#        
# 
# 
# Plot a choropleth map
mf_map(x = mtq, var = "MED", type = "choro",
       pal = "Dark Mint", 
       breaks = "quantile", 
       nbreaks = 6, 
       leg_title = "Median Income\n(euros)", 
       leg_val_rnd = -2,
       leg_pos = "topright")     
# 
# 
# 
#
# 
# Plot a layout elements
mf_title("Wealth in Martinique, 2015")
mf_credits("T. Giraud\nSources: INSEE & IGN, 2018")
mf_scale(size = 5)
mf_arrow('topleft')
```
```{r c, message = FALSE, results='hide', fig.width = 5, fig.height=5, eval=TRUE, echo=FALSE}
```
</div>

## Example {.smaller}

<div class="columns-2">
```{r d, eval=FALSE}
mtq <- mf_get_mtq()
# Start a map using a theme  
mf_init(x = mtq, theme = "dark")
#     
# 
# 
# Plot a choropleth map
mf_map(x = mtq, var = "MED", type = "choro",
       pal = "Dark Mint", 
       breaks = "quantile", 
       nbreaks = 6, 
       leg_title = "Median Income\n(euros)", 
       leg_val_rnd = -2,
       leg_pos = "topright", 
       add = TRUE)     
# 
# 
#
# 
# Plot a layout elements
mf_title("Wealth in Martinique, 2015")
mf_credits("T. Giraud\nSources: INSEE & IGN, 2018")
mf_scale(size = 5)
mf_arrow('topleft')
```
```{r d, message = FALSE, results='hide', fig.width = 5, fig.height=5, eval=TRUE, echo=FALSE}
```
</div>

## Example {.smaller}

<div class="columns-2">
```{r e, eval=FALSE}
mtq <- mf_get_mtq()
# Start a map using a theme  
mf_init(x = mtq, theme = "dark")
#     
# Plot a shadow
mf_shadow(mtq, col = "grey10", add = TRUE)
# Plot a choropleth map
mf_map(x = mtq, var = "MED", type = "choro",
       pal = "Dark Mint", 
       breaks = "quantile", 
       nbreaks = 6, 
       leg_title = "Median Income\n(euros)", 
       leg_val_rnd = -2,
       leg_pos = "topright", 
       add = TRUE)     
# 
# 
#
# 
# Plot a layout elements
mf_title("Wealth in Martinique, 2015")
mf_credits("T. Giraud\nSources: INSEE & IGN, 2018")
mf_scale(size = 5)
mf_arrow('topleft')
```
```{r e, message = FALSE, results='hide', fig.width = 5, fig.height=5, eval=TRUE, echo=FALSE}
```
</div>



## Example {.smaller}

<div class="columns-2">
```{r f, eval=FALSE}
mtq <- mf_get_mtq()
# Start a map using a theme  
mf_init(x = mtq, theme = "dark")
#     
# Plot a shadow
mf_shadow(mtq, col = "grey10", add = TRUE)
# Plot a choropleth map
mf_map(x = mtq, var = "MED", type = "choro",
       pal = "Dark Mint", 
       breaks = "quantile", 
       nbreaks = 6, 
       leg_title = "Median Income\n(euros)", 
       leg_val_rnd = -2,
       leg_pos = "topright", 
       add = TRUE)     
# Add an inset world map
mf_inset_on(x = "worldmap", pos = "right")
mf_worldmap(mtq, col = "#0E3F5C")
mf_inset_off()
# Plot a layout elements
mf_title("Wealth in Martinique, 2015")
mf_credits("T. Giraud\nSources: INSEE & IGN, 2018")
mf_scale(size = 5)
mf_arrow('topleft')
```

```{r f, message = FALSE, results='hide', fig.width = 5, fig.height=5, eval=TRUE, echo=FALSE}
```
</div>



## Example {.smaller}

<div class="columns-2">
```{r, results='hide', message = FALSE, echo = FALSE}
library(mapsf)
```

```{r, message = FALSE, results='hide', fig.width = 5, fig.height=5}
mtq <- mf_get_mtq()
# Start a map using a theme and extra margins 
mf_init(x = mtq, theme = "dark",
        expandBB = c(0,0,0,.3)) 
# Plot a shadow
mf_shadow(mtq, col = "grey10", add = TRUE)
# Plot a choropleth map
mf_map(x = mtq, var = "MED", type = "choro",
       pal = "Dark Mint", 
       breaks = "quantile", 
       nbreaks = 6, 
       leg_title = "Median Income\n(euros)", 
       leg_val_rnd = -2,
       leg_pos = "topright",
       add = TRUE)
# Add an inset world map
mf_inset_on(x = "worldmap", pos = "right")
mf_worldmap(mtq, col = "#0E3F5C")
mf_inset_off()
# Plot a layout elements
mf_title("Wealth in Martinique, 2015")
mf_credits("T. Giraud\nSources: INSEE & IGN, 2018")
mf_scale(size = 5)
mf_arrow('topleft')


```

</div>


## Insets - [Vignette](https://riatelab.github.io/mapsf/articles/web_only/how_to_create_inset_maps.html) {.smaller}


<div class="columns-2">
```{r, message = FALSE, results='hide', fig.width = 3.5, fig.height=3.5}
mf_map(mtq)
mf_inset_on(x = mtq[1, ], cex = .3)
mf_map(mtq[1, ])
mf_inset_off()


mf_map(mtq)
mf_inset_on(x = "worldmap", pos = "bottomleft")
mf_worldmap(x = mtq)
mf_inset_off()
```
</div>

## Insets - [Vignette](https://riatelab.github.io/mapsf/articles/web_only/how_to_create_inset_maps.html) {.smaller}

<center>
![](img/insethist.png)
</center>

## Themes - [Vignette](https://riatelab.github.io/mapsf/articles/web_only/how_to_use_themes.html) {.smaller}

<div class="columns-3">
```{r, message = FALSE, results='hide', fig.width = 2, fig.height=2.7, echo = FALSE}
mf_theme("default")
mf_map(mtq)
mf_title("default")

mf_theme("jsk")
mf_map(mtq)
mf_title("jsk")

mf_theme("green")
mf_map(mtq)
mf_title("green")

mf_theme("darkula")
mf_map(mtq)
mf_title("darkula")

mf_theme("ink")
mf_map(mtq)
mf_title("ink")

mf_theme("candy")
mf_map(mtq)
mf_title("candy")

mf_theme("dark")
mf_map(mtq)
mf_title("dark")

mf_theme("barcelona")
mf_map(mtq)
mf_title("barcelona")

mf_theme("nevermind")
mf_map(mtq)
mf_title("nevermind")

mf_theme(bg = "#aad3df", fg = "#393939", mar = c(0,0,0,0), 
         tab = TRUE, inner = TRUE, line = 1.3, pos = "center", 
         cex = 1.1, font = 3)
mf_map(mtq)
mf_title("Custom")


```

</div>




## Raster {.smaller}

<div class="columns-2">

```{r raster, eval = FALSE}
library(maptiles)
osm <- get_tiles(
  x = mtq,
  provider = "Stamen.TerrainBackground", 
  crop= TRUE, zoom = 11
)
mf_theme("default", bg = "#99b3cc")
mf_raster(osm)
mf_map(mtq, border = "tomato4", col = NA, 
       add = TRUE)
mf_scale()
mf_title("mf_raster()")
mf_credits(
  txt = get_credit("Stamen.TerrainBackground")
)
```
<br/>
<br>
<br>
<br>
<br>
<br>



```{r raster1, message = FALSE, results='hide', fig.width = 4.7, fig.height=5, eval=TRUE, echo=FALSE}
library(maptiles)
osm <- get_tiles(mtq, provider = "Stamen.TerrainBackground", crop= TRUE, zoom = 11, cachedir = "img")
osm[is.na(osm)] <- 255
mf_theme("default", bg = "#99b3cc")
mf_raster(osm)
mf_map(mtq, border = "tomato4", col = NA, add = TRUE)
mf_scale()
mf_title("mf_raster()")
mf_credits(txt = get_credit("Stamen.TerrainBackground"))


```

</div>

## New!  {.smaller}

<div class="columns-2">
```{r bg, eval=FALSE, par.family = "B612"}
library(mapsf)
mtq <- mf_get_mtq()
mf_init(mtq)
mf_background("img/blackboard.jpg")
mf_map(mtq, lwd = 3, col = NA, 
       border = "white", add = TRUE)
mf_credits(
  txt = "Photo by Noita Digital on Unsplash",
  col = "white"
)
mf_title("mf_background()")
```

<br>
<br>
```{r bg, message = FALSE, results='hide', fig.width = 5, fig.height=5, eval=TRUE, echo=FALSE}
```
</div>


## Next features

- Legends for rasters (continuous, classes, categories)  

- Horizontal layout for legends  

- [You tell me!](https://github.com/riatelab/mapsf/issues)  

## Development {.smaller}

A minimal number of well-known dependencies


```{r, echo = FALSE, eval = FALSE, results='hide'}
library(deepdep)
dep <- deepdep("mapsf", depth = 6)
png("img/deps.png", width  = 600, height = 450)
plot_dependencies(dep, "circular", same_level = TRUE, reverse = TRUE, main = 'toto')
dev.off()

```
<center>
<img src="img/deps.png" width=600px class="center"/>
</center>

## Development

- CI/CD with [GitHub Actions](https://github.com/riatelab/mapsf/actions)   

- Tests with `tinytest` package, coverage with [Codecov](https://app.codecov.io/gh/riatelab/mapsf)     

- Informative commit messages with [conventional commits](https://www.conventionalcommits.org/en/v1.0.0-beta.3/)



## [riatelab.github.io/mapsf](https://riatelab.github.io/mapsf/)

<center>
[<img src="img/website.png" width=800px />](https://riatelab.github.io/mapsf/)
</center>







## Thank You

<img src="img/logo.png" width=100px />  [riatelab.github.io/mapsf](https://riatelab.github.io/mapsf)  
</br>
<img src="img/prlogo.png" width=50px />  [github.com/rCarto/foss4g2021](https://github.com/rCarto/foss4g2021)  
</br>
<img src="img/ghlogo.png" width=50px />  [github.com/riatelab/mapsf](https://github.com/riatelab/mapsf)  
</br>
<img src="img/twlogo.png" width=50px />  [\@rgeomatic](http://twitter.com/rgeomatic)  
</br>
<img src="img/hylogo.png" width=50px />  [rgeomatic.hypotheses.org](https://rgeomatic.hypotheses.org/)


</div>
